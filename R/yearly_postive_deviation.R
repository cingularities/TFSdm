...
yearly_positive_deviation <- function(sma_performance_yearly,se_performance_yearly,estimates_raw,status) {
  ...
  se_performance <- se_performance_yearly %>% select(-c(1,3)) %>% #Selects column
    ...
    left_join(estimates %>% select(PORTAL.NAME,PORTAL.SERVER,JobAddress, City, State, Postal, Email, InstallationDate, JobName, MonitorCode, YearlyEstimate), by = c("PORTAL.NAME","PORTAL.SERVER")) #joins with performance residence
  ...
  se_sma_deviation <- se_sma_bind %>%
    ...
    left_join(status %>% select(PORTAL.NAME,STATUS, CATEGORY), by = "PORTAL.NAME")%>% #joins recent down dm list with current portal status
